...
backpropagation.sgd <- function(model, dataset, batch.size=1, eta=0.1, 
				threshold=1e-3, k=10, mult.factor=0.99, lambda=0,
	...
	while (squared_error > threshold && 
		...
		for (batch.id in 1:nbatches) {
		   ...
		   for (p in start:end) {
			...
			# Calculando o erro e somando norma dos pesos
			...
			Wrl2 = model$layers$output
			Wrl2[,ncol]=0
			delta_o = error * model$df.dnet(f=fwd$f_net_o)
			delta_ow = lambda * Wrl2
			...
			for (layer.id in (length(model$layers.sizes)-2):1) {
				...
				W_curr = model$layers$hidden[[layer.id]]
				W_curr[,ncol] = 0
				# Hidden Layer n
				delta_h[[layer.id]] = 
					model$df.dnet(f=fwd$f_net_h[[layer.id]]) * 
						sum(as.vector(delta)%*%W)
				delta_hw[[layer.id]] = lambda*W_curr
		   ...
		   model$layers$output = model$layers$output + deltaW_Output - delta_ow 

		   for (layer.id in (length(model$layers.sizes)-2):1) {
				model$layers$hidden[[layer.id]] =
					model$layers$hidden[[layer.id]] + deltaW_Hidden[[layer.id]] - delta_hw[[layer.id]]
		   }
		}
...
iris.test <- function(layers.sizes = c(4, 3, 3), batch.size=25,
			...
			error_threshold = 1e-7, momentum.alpha = 0.9, lambda = 0,
...
iris.train.test <- function(layers.sizes = c(4, 3, 3), batch.size=25,
		      	eta=0.5, threshold=1e-7, debug=F,
			error_threshold = 1e-7, momentum.alpha = 0.9, lambda=0,
			dropout.probability = 0.5, mult.factor = 0.99) {
	...
	model = mlp.architecture(layers.sizes = layers.sizes, f.net=sigmoid, df.dnet=diff.sigmoid)
	trained.model = backpropagation.sgd(model, trainset, 
				batch.size=batch.size,
				eta=eta, threshold=threshold,
				error_threshold = error_threshold,
				momentum.alpha=momentum.alpha,
				...
				mult.factor = mult.factor,
				lambda=lambda)
	return (mlp.test(trained.model, testset, debug=debug))
}
